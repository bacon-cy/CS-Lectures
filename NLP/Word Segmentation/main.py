...
pointer = 0  # the pointer points out the current scanning end
while pointer < input_string_len:
    for length in range(dict_words_max_len + 1, 0, -1):
        if pointer + length > input_string_len:
            continue  # meaning that the testing-argument would be null or repeated, thus, skip it
        str = input_string[pointer:(pointer + length)]  # the current testing-segment
        print(pointer, length, str)
        exists = dict.get(str + '\n', -1)
        ...
        if is_numbers_or_alphabets(input_string[pointer]):
            tmp_pointer = pointer + 1
            while is_numbers_or_alphabets(input_string[tmp_pointer]):
                tmp_pointer += 1
            # input_string[pointer : tmp_pointer] is numbers and english words
            output_list.append(input_string[pointer : tmp_pointer])
            pointer = tmp_pointer
            break
        if length == 1:
            output_list.append(input_string[pointer])  # if there is none of word end with input_string[pointer],
